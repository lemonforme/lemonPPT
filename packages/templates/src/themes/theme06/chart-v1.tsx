...
export interface Theme06ChartV1Insight {
  value?: string;
  label?: string;
  description?: string;
}

export interface Theme06ChartV1Props {
  imageUrl?: string;
  kicker?: string;
  title: string;
  subtitle?: string;
  ...
  conclusion?: Theme06ChartV1Insight;
  _slideIdx?: number;
  _editable?: boolean;
  [key: string]: unknown;
}

export const theme06ChartV1Meta: LayoutMeta = {
  id: 'theme06_chart_v1',
  theme: 'theme06',
  role: 'chart',
  displayName: 'Theme 06 图谱图表页',
  description: '柱状/折线趋势图 + 右侧洞察卡',
  needsMedia: true,
  tags: ['chart', 'bar', 'line', 'atlas'],
  contentShape: 'generic-chart',
};

export const theme06ChartV1Schema: PropsSchema = {
  fields: [
...
function buildOption(type: 'bar' | 'line', labels: string[], data: number[]): Record<string, unknown> {
  ...
  const seriesData = data.map((value, index) => ({
    value,
    itemStyle: {
      color: index === topIndex ? 'var(--lp-accent)' : 'var(--lp-accent-2)',
...
export function Theme06ChartV1(props: Theme06ChartV1Props): ReactNode {
