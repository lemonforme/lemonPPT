...
export interface Theme07AvgTicketV1Props {
  imageUrl?: string;
  kicker?: string;
  title: string;
  subtitle?: string;
...
export const theme07AvgTicketV1Meta: LayoutMeta = {
  id: 'theme07_avg_ticket_v1',
  theme: 'theme07',
  role: 'chart',
  displayName: 'Theme 07 平均交易额',
  description: '区间直方图展示平均交易额或交易规模分布',
  needsMedia: true,
  tags: ['chart', 'bar', 'histogram', 'ticket'],
  contentShape: 'generic-chart',
};

export const theme07AvgTicketV1Schema: PropsSchema = {
  fields: [
    { key: 'imageUrl', label: '背景图片', type: 'image' },
    { key: 'kicker', label: '顶部标签', type: 'text', inlineEditable: true, defaultValue: 'AVG TICKET' },
    { key: 'title', label: '标题', type: 'text', inlineEditable: true, defaultValue: '平均交易额分布' },
    { key: 'subtitle', label: '副标题', type: 'textarea', inlineEditable: true, defaultValue: '不同规模区间的交易数量占比' },
    {
      key: 'intervals',
      label: '区间标签',
      type: 'array',
...
function normalizeStringArray(input: unknown): string[] {
  ...
  return input.map((item) => (typeof item === 'string' ? item : (item as { item?: string }).item ?? '')).filter((item) => item !== '');
}

function normalizeNumberArray(input: unknown): number[] {
  if (!Array.isArray(input)) return [];
  return input.map((value) => (typeof value === 'number' ? value : Number((value as { item?: number }).item ?? 0) || 0));
}
...
function buildOption(labels: string[], values: number[]): Record<string, unknown> {
  ...
  return {
    grid: { top: 32, right: 24, bottom: 56, left: 56, containLabel: false },
    tooltip: {
      trigger: 'axis',
...
export function Theme07AvgTicketV1(props: Theme07AvgTicketV1Props): ReactNode {
  const { imageUrl, kicker, title, subtitle, intervals = [], values = [], unit, conclusion, _slideIdx, _editable } = props;
