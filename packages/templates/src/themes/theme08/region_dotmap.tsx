...
export interface Theme08Region_dotmapProps { kicker?: string; title?: string; subtitle?: string; items?: any[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }

const DEFAULT_ITEMS: any[] = [{"title":"北美","num":"3","unit":"×","label":"能力层级","desc":"以黑金视觉统一表达核心能力"},{"title":"欧洲","num":"6","unit":"×","label":"能力层级","desc":"强调端到端闭环与可复用性"},{"title":"东亚","num":"9","unit":"×","label":"能力层级","desc":"面向高端发布的实验性叙事"},{"title":"其他","num":"12","unit":"×","label":"能力层级","desc":"用荧光金突出关键指标"}];

export const theme08Region_dotmapMeta: LayoutMeta = buildMeta({
  id: 'theme08_region_dotmap_v1',
  role: 'region',
  displayName: '地域·点阵',
  description: '地域页：区域点阵分布',
  contentShape: 'region',
  tags: ['black-gold', 'experimental'],
});

export const theme08Region_dotmapSchema: PropsSchema = {
  fields: [
    ...commonFields({ kicker: 'BLACK GOLD', title: '地域·点阵', subtitle: '' }),
...
export function Theme08Region_dotmap(props: Theme08Region_dotmapProps): ReactNode {
  const { kicker, title = '标题', subtitle, items = DEFAULT_ITEMS, footerLeft, footerRight, _slideIdx, _editable } = props;
  const COLS = 4;
  return (
      <T08Frame className="t08-cards grid4">
