...
export interface Theme08Resource_mapProps { kicker?: string; title?: string; subtitle?: string; items?: any[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }

const DEFAULT_ITEMS: any[] = [{"name":"接入层","desc":"统一入口与多端适配。"},{"name":"调度层","desc":"算力与任务的弹性编排。"},{"name":"模型层","desc":"多模型路由与评测。"},{"name":"数据层","desc":"高质量语料与特征底座。"}];

export const theme08Resource_mapMeta: LayoutMeta = buildMeta({
  id: 'theme08_resource_map_v1',
  role: 'resource_map',
  displayName: '资源绑定',
  description: '资本页：资源绑定链路',
  contentShape: 'resource_map',
  tags: ['black-gold', 'experimental'],
});

export const theme08Resource_mapSchema: PropsSchema = {
  fields: [
    ...commonFields({ kicker: 'BLACK GOLD', title: '资源绑定', subtitle: '' }),
    ...([{"key":"items","label":"节点","type":"array","itemSchema":[{"key":"name","label":"名称","type":"text"},{"key":"desc","label":"描述","type":"text"}]}] as any),
  ],
};

export function Theme08Resource_map(props: Theme08Resource_mapProps): ReactNode {
