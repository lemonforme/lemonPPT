// lemonPPT - theme08 黑金实验 · 排名列表
// 原创实现，不复制 Dashi theme08

import type { LayoutMeta, PropsSchema } from '@lemonppt/core';
import type { ReactNode } from 'react';
import { EditableField } from '../../editable-field.js';
import { Theme08SlideBg } from './slide-bg.js';
import { Theme08MiniBars } from './decorations.js';
import { Theme08IconChip } from './theme08-icons.js';

export interface Theme08RankingV1Item {
  name: string;
  value: string;
  accent?: boolean;
}

export interface Theme08RankingV1Props {
  kicker?: string;
  title: string;
  subtitle?: string;
  unit?: string;
  items?: Theme08RankingV1Item[];
  footnoteLeft?: string;
  footnoteRight?: string;
  _slideIdx?: number;
  _editable?: boolean;
  [key: string]: unknown;
}

export const theme08RankingV1Meta: LayoutMeta = {
  id: 'theme08_ranking_v1',
  theme: 'theme08',
  role: 'metric',
  displayName: 'Theme 08 排名列表',
  description: '纵向排名条目 + 序号 + 数值，适合榜单/排行',
  needsMedia: false,
  tags: ['ranking', 'list', 'black-gold'],
  contentShape: 'ranking',
};

export const theme08RankingV1Schema: PropsSchema = {
  fields: [
    { key: 'kicker', label: '顶部标签', type: 'text', inlineEditable: true, defaultValue: 'TOP FUNDED' },
    { key: 'title', label: '标题', type: 'text', inlineEditable: true, defaultValue: 'Top 融资公司' },
    { key: 'subtitle', label: '副标题', type: 'textarea', inlineEditable: true, defaultValue: '头部公司融资额显著领先。' },
    { key: 'unit', label: '单位', type: 'text', inlineEditable: true, defaultValue: '亿$' },
    {
      key: 'items',
      label: '排名条目',
      type: 'array',
      minItems: 1,
      maxItems: 8,
      defaultValue: [
        { name: 'OpenAI', value: '66', accent: true },
        { name: 'Anthropic', value: '65', accent: true },
        { name: 'xAI', value: '50', accent: true },
        { name: 'CoreWeave', value: '11' },
        { name: 'SSI', value: '10' },
        { name: 'Scale AI', value: '10' },
      ],
      itemSchema: [
        { key: 'name', label: '名称', type: 'text' },
        { key: 'value', label: '数值', type: 'text' },
        { key: 'accent', label: '高亮', type: 'boolean' },
      ],
    },
    { key: 'footnoteLeft', label: '左页脚', type: 'text', inlineEditable: true, defaultValue: 'lemonPPT · 2026' },
    { key: 'footnoteRight', label: '右页脚', type: 'text', inlineEditable: true, defaultValue: '10' },
  ],
};

export function Theme08RankingV1(props: Theme08RankingV1Props): ReactNode {
  const { kicker, title, subtitle, unit, items = [], footnoteLeft, footnoteRight, _slideIdx, _editable } = props;
  const valid = (items || []).slice(0, 8);
  return (
    <div className="lp-slide lp-theme08 lp-theme08-ranking-page">
      <Theme08SlideBg />
      <div className="lp-theme08-page">
        <div className="lp-theme08-section-header lp-rise">
          <Theme08IconChip name="chart" size={40} />
          {kicker && <div className="lp-theme08-kicker"><EditableField prop="kicker" slideIdx={_slideIdx} editable={_editable} as="span">{kicker}</EditableField></div>}
          <EditableField prop="title" slideIdx={_slideIdx} editable={_editable} as="h2" className="lp-theme08-title">{title}</EditableField>
          {subtitle && <EditableField prop="subtitle" slideIdx={_slideIdx} editable={_editable} as="p" className="lp-theme08-subtitle">{subtitle}</EditableField>}
        </div>
        <div className="lp-theme08-body">
          <div className="lp-theme08-ranking lp-rise">
            {valid.map((it, i) => (
              <div key={i} className={`lp-theme08-ranking-row ${it.accent ? 'lp-focus' : ''}`} style={{ animationDelay: `${i * 50}ms` }}>
                <div className="lp-theme08-ranking-pos">{String(i + 1).padStart(2, '0')}</div>
                <div className="lp-theme08-ranking-name"><EditableField prop={`items.${i}.name`} slideIdx={_slideIdx} editable={_editable} as="span">{it.name}</EditableField></div>
                <div className="lp-theme08-ranking-val">
                  <EditableField prop={`items.${i}.value`} slideIdx={_slideIdx} editable={_editable} as="span">{it.value}</EditableField>
                  {unit && <span className="lp-theme08-metric-unit">{unit}</span>}
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
      <div className="lp-theme08-glow-line" aria-hidden="true" />
      <div className="lp-theme08-footer">
        <span className="lp-theme08-footer-left">{footnoteLeft && <EditableField prop="footnoteLeft" slideIdx={_slideIdx} editable={_editable} as="span">{footnoteLeft}</EditableField>}</span>
        <span className="lp-theme08-footer-right">{footnoteRight && <EditableField prop="footnoteRight" slideIdx={_slideIdx} editable={_editable} as="span">{footnoteRight}</EditableField>}</span>
      </div>
      <Theme08MiniBars count={20} />
    </div>
  );
}
