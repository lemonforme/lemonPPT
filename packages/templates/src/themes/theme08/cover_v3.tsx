...
export interface Theme08Cover_v3Props { tag?: string; title?: string; subtitle?: string; stats?: { value: string; unit?: string; label: string }[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }

export const theme08Cover_v3Meta: LayoutMeta = buildMeta({
  id: 'theme08_cover_v3',
  role: 'cover',
  displayName: '黑金封面·极简',
  description: '黑金实验封面：底部极简标题 + 手绘装饰',
  contentShape: 'cover',
  tags: ['black-gold', 'experimental'],
});

export const theme08Cover_v3Schema: PropsSchema = {
  fields: [
    ...footerFields(),
...
export function Theme08Cover_v3(props: Theme08Cover_v3Props): ReactNode {
  const { tag, title = '黑金实验 · 全新一页', subtitle, footerLeft, footerRight, _slideIdx, _editable } = props;
  
  return (
      <T08Frame className="t08-cover minimal">
