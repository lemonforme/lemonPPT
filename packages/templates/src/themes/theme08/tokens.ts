 ...
 * 本文件经 `packages/templates/src/index.ts` 导出，被 `@lemonppt/renderer` 的 render.tsx 实际调用，
 * 是页面上真正生效的 CSS 变量来源。修改 theme08 配色请**优先改本文件**。
 * `packages/themes/src/theme08/tokens.ts` 为镜像副本（供 getTheme 元数据与已发布包 API 使用），
 * 两者必须保持一致，改完请同步：
