...
export interface Theme08Cover_v2Props { tag?: string; title?: string; subtitle?: string; stats?: { value: string; unit?: string; label: string }[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }
...
export const theme08Cover_v2Meta: LayoutMeta = buildMeta({
  id: 'theme08_cover_v2',
  role: 'cover',
  displayName: '黑金封面·居中',
  description: '黑金实验封面：居中大标题 + 荧光金指标墙',
  contentShape: 'cover',
  tags: ['black-gold', 'experimental'],
});

export const theme08Cover_v2Schema: PropsSchema = {
  fields: [
    ...footerFields(),
...
export function Theme08Cover_v2(props: Theme08Cover_v2Props): ReactNode {
  const { tag, title = '黑金实验 · 全新一页', subtitle, stats = DEFAULT_ITEMS, footerLeft, footerRight, _slideIdx, _editable } = props;
  
  return (
      <T08Frame className="t08-cover centered">
