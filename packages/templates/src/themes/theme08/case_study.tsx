...
export interface Theme08Case_studyProps { kicker?: string; title?: string; subtitle?: string; items?: any[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }
...
export const theme08Case_studyMeta: LayoutMeta = buildMeta({
  id: 'theme08_case_study_v1',
  role: 'case',
  displayName: '案例·深读',
  description: '案例页：左右深读案例',
  contentShape: 'case',
  tags: ['black-gold', 'experimental'],
});

export const theme08Case_studySchema: PropsSchema = {
  fields: [
    ...commonFields({ kicker: 'BLACK GOLD', title: '案例·深读', subtitle: '' }),
...
export function Theme08Case_study(props: Theme08Case_studyProps): ReactNode {
  const { kicker, title = '标题', subtitle, items = DEFAULT_ITEMS, footerLeft, footerRight, _slideIdx, _editable } = props;
  const COLS = 2;
  return (
      <T08Frame className="t08-cards study">
