...
export interface Theme08ArchitectureProps { kicker?: string; title?: string; subtitle?: string; items?: any[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }

const DEFAULT_ITEMS: any[] = [{"name":"接入层","desc":"统一入口与多端适配。"},{"name":"调度层","desc":"算力与任务的弹性编排。"},{"name":"模型层","desc":"多模型路由与评测。"},{"name":"数据层","desc":"高质量语料与特征底座。"},{"name":"安全层","desc":"对齐、护栏与审计。"},{"name":"应用层","desc":"场景化交付与闭环。"}];

export const theme08ArchitectureMeta: LayoutMeta = buildMeta({
  id: 'theme08_architecture_v1',
  role: 'architecture',
  displayName: '架构·分层',
  description: '架构页：分层架构流',
  contentShape: 'architecture',
  tags: ['black-gold', 'experimental'],
});

export const theme08ArchitectureSchema: PropsSchema = {
  fields: [
    ...commonFields({ kicker: 'BLACK GOLD', title: '架构·分层', subtitle: '' }),
    ...([{"key":"items","label":"节点","type":"array","itemSchema":[{"key":"name","label":"名称","type":"text"},{"key":"desc","label":"描述","type":"text"}]}] as any),
  ],
};

export function Theme08Architecture(props: Theme08ArchitectureProps): ReactNode {
