...
export interface Theme08StrategyV1Item {
  title: string;
  desc?: string;
}

export interface Theme08StrategyV1Props {
  kicker?: string;
  title: string;
  subtitle?: string;
  items?: Theme08StrategyV1Item[];
  footnoteLeft?: string;
...
export const theme08StrategyV1Meta: LayoutMeta = {
  id: 'theme08_strategy_v1',
  theme: 'theme08',
  role: 'feature',
  displayName: 'Theme 08 策略卡',
  description: '标题 + 三栏策略卡（带序号徽标），适合打法/策略',
  needsMedia: false,
  tags: ['strategy', 'cards', 'black-gold'],
  contentShape: 'strategy',
};

export const theme08StrategyV1Schema: PropsSchema = {
  fields: [
    { key: 'kicker', label: '顶部标签', type: 'text', inlineEditable: true, defaultValue: 'PLAYBOOK' },
    { key: 'title', label: '标题', type: 'text', inlineEditable: true, defaultValue: '三大落地策略' },
    { key: 'subtitle', label: '副标题', type: 'textarea', inlineEditable: true, defaultValue: '聚焦、协同、开放，三招打透市场。' },
    {
      key: 'items',
      label: '策略卡',
      type: 'array',
      ...
      maxItems: 4,
      defaultValue: [
        { title: '聚焦核心场景', desc: '集中资源打透 2-3 个高价值垂直场景。' },
        { title: '生态协同', desc: '与算力/数据伙伴共建，降低边际成本。' },
        { title: '开放平台', desc: '以开发者网络放大网络效应。' },
      ],
      itemSchema: [
    ...
    { key: 'footnoteRight', label: '右页脚', type: 'text', inlineEditable: true, defaultValue: '77' },
  ],
};

export function Theme08StrategyV1(props: Theme08StrategyV1Props): ReactNode {
  const { kicker, title, subtitle, items = [], footnoteLeft, footnoteRight, _slideIdx, _editable } = props;
  const valid = (items || []).slice(0, 4);
  return (
    <div className="lp-slide lp-theme08 lp-theme08-strategy-page">
      <Theme08SlideBg />
      <div className="lp-theme08-page">
        ...
        <div className="lp-theme08-body">
          <div className="lp-theme08-strategy lp-rise">
            {valid.map((it, i) => (
              <div key={i} className="lp-theme08-card lp-theme08-card-pad lp-theme08-strategy-card" style={{ animationDelay: `${i * 70}ms` }}>
                <div className="lp-theme08-strategy-top">
                  <span className="lp-theme08-list-index" style={{ minWidth: 32 }}>S{String(i + 1).padStart(2, '0')}</span>
