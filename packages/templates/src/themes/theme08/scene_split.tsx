...
export interface Theme08Scene_splitProps { kicker?: string; title?: string; subtitle?: string; items?: any[]; footerLeft?: string; footerRight?: string; _slideIdx?: number; _editable?: boolean; [key: string]: unknown; }

const DEFAULT_ITEMS: any[] = [{"title":"算力调度","num":"3","unit":"×","label":"能力层级","desc":"以黑金视觉统一表达核心能力"},{"title":"数据底座","num":"6","unit":"×","label":"能力层级","desc":"强调端到端闭环与可复用性"}];

export const theme08Scene_splitMeta: LayoutMeta = buildMeta({
  id: 'theme08_scene_split_v1',
  role: 'scene_split',
  displayName: '场景占比',
  description: '专题页：场景左右占比',
  contentShape: 'scene_split',
  tags: ['black-gold', 'experimental'],
});

export const theme08Scene_splitSchema: PropsSchema = {
  fields: [
    ...commonFields({ kicker: 'BLACK GOLD', title: '场景占比', subtitle: '' }),
...
export function Theme08Scene_split(props: Theme08Scene_splitProps): ReactNode {
  const { kicker, title = '标题', subtitle, items = DEFAULT_ITEMS, footerLeft, footerRight, _slideIdx, _editable } = props;
  const COLS = 2;
  return (
      <T08Frame className="t08-cards cards2">
